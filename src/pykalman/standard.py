...
def _arg_or_default(arg, default, dim, name):
    if arg is None:
        result = torch.tensor(default,dtype=torch.float64)
    else:
        result = arg
...
def _determine_dimensionality(variables, default):
    ...
    if len(candidates) == 0:
        ...
        if not torch.all(torch.tensor(candidates,dtype=torch.float64) == candidates[0]):
            raise ValueError(
                "The shape of all " +
...
def _loglikelihoods(observation_matrices, observation_offsets,
    ...
    loglikelihoods = torch.zeros(n_timesteps,dtype=torch.float64)
    for t in range(n_timesteps):
        observation = observations[t]
        #if not torch.tensor(np.ma.getmask(observation).astype(int)).byte().any():
        #if not observation.byte().any():
        ...
        predicted_observation_mean = (
            torch.dot(observation_matrix,
                   predicted_state_mean)
            + observation_offset
        )
        predicted_observation_covariance = (
            torch.dot(observation_matrix,
                   torch.dot(predicted_state_covariance,
                          observation_matrix.T))
            + observation_covariance
        )
        loglikelihoods[t] = log_multivariate_normal_density(
            observation[torch.newaxis, :],
            predicted_observation_mean[torch.newaxis, :],
            predicted_observation_covariance[torch.newaxis, :, :]
        )
...
def _filter_predict(transition_matrix, transition_covariance,
    ...
    predicted_state_covariance = (
        torch.tensor(torch.matmul(transition_matrix,
...
def _filter_correct(observation_matrix, observation_covariance,
    ...
    #if not torch.tensor(np.ma.getmask(observation).astype(int)).byte().any():
    #if not observation.byte().any():
    observation_matrix_t = torch.t(observation_matrix)
    predicted_observation_mean = (
        torch.tensor(torch.matmul(observation_matrix,
               predicted_state_mean),dtype=torch.float64)
        + observation_offset
    )
    predicted_observation_covariance = (
        torch.tensor(torch.matmul(observation_matrix,
            torch.matmul(predicted_state_covariance,
                      observation_matrix_t)),dtype=torch.float64)
        + observation_covariance
    )

    kalman_gain = (
        torch.tensor(torch.matmul(predicted_state_covariance,
               torch.matmul(observation_matrix_t,
    ...
    corrected_state_covariance = (
        predicted_state_covariance
        - torch.tensor(torch.matmul(kalman_gain,
                 torch.matmul(observation_matrix,
    ...
    #    kalman_gain = torch.zeros((n_dim_state, n_dim_obs),dtype=torch.float64)
...
def _filter(transition_matrices, observation_matrices, transition_covariance,
    ...
    predicted_state_means = torch.zeros((n_timesteps, n_dim_state),dtype=torch.float64)
    predicted_state_covariances = torch.zeros(
        (n_timesteps, n_dim_state, n_dim_state),dtype=torch.float64
    )
    kalman_gains = torch.zeros((n_timesteps, n_dim_state, n_dim_obs),dtype=torch.float64)
    filtered_state_means = torch.zeros((n_timesteps, n_dim_state),dtype=torch.float64)
    filtered_state_covariances = torch.zeros(
        (n_timesteps, n_dim_state, n_dim_state),dtype=torch.float64
    )
...
def _smooth_update(transition_matrix, filtered_state_mean,
    ...
    kalman_smoothing_gain = (
        torch.tensor(torch.matmul(filtered_state_covariance,
               torch.matmul(torch.t(transition_matrix),
                      torch.pinverse(predicted_state_covariance))),dtype=torch.float64)
    )

    #next_smoothed_state_mean = torch.tensor(next_smoothed_state_mean,dtype=torch.float64)
    #predicted_state_mean = torch.tensor(predicted_state_mean,dtype=torch.float64)
    ...
    smoothed_state_mean = (
        filtered_state_mean
        + torch.tensor(torch.matmul(kalman_smoothing_gain,
...
def _smooth(transition_matrices, filtered_state_means,
    ...
    smoothed_state_means = torch.zeros((n_timesteps, n_dim_state),dtype=torch.float64)
    smoothed_state_covariances = torch.zeros((n_timesteps, n_dim_state,
                                           n_dim_state),dtype=torch.float64)
    kalman_smoothing_gains = torch.zeros((n_timesteps - 1, n_dim_state,
                                       n_dim_state),dtype=torch.float64)
...
def _smooth_pair(smoothed_state_covariances, kalman_smoothing_gain):
    ...
    pairwise_covariances = torch.zeros((n_timesteps, n_dim_state, n_dim_state),dtype=torch.float64)
    for t in range(1, n_timesteps):
        pairwise_covariances[t] = torch.tensor(
            torch.matmul(smoothed_state_covariances[t],
                   torch.t(kalman_smoothing_gain[t - 1]))
        ,dtype=torch.float64)
...
def _em_observation_matrix(observations, observation_offsets,
    ...
    res1 = torch.zeros((n_dim_obs, n_dim_state),dtype=torch.float64)
    res2 = torch.zeros((n_dim_state, n_dim_state),dtype=torch.float64)
    for t in range(n_timesteps):
        #if not observations[t].byte().any():
        #if not torch.tensor(np.ma.getmask(observations[t]).astype(int)).byte().any():
        observation_offset = _last_dims(observation_offsets, t, ndims=1)
        res1 += torch.ger(observations[t] - observation_offset,
                         smoothed_state_means[t])
        res2 += (
            smoothed_state_covariances[t]
            + torch.ger(smoothed_state_means[t], 
                smoothed_state_means[t])
        )
    return torch.dot(res1, torch.pinverse(res2))
...
def _em_observation_covariance(observations, observation_offsets,
    ...
    _, n_dim_state = smoothed_state_means.shape
    n_timesteps, n_dim_obs = observations.shape
    res = torch.zeros((n_dim_obs, n_dim_obs),dtype=torch.float64)
    n_obs = 0
    for t in range(n_timesteps):
        #if not torch.tensor(np.ma.getmask(observations[t]).astype(int)).byte().any():
        #if not observations[t].byte().any():
        ...
        err = torch.tensor(
            torch.tensor(observations[t],dtype=torch.float64)
            - torch.tensor(torch.matmul(transition_matrix, smoothed_state_means[t]))
            - transition_offset
        ,dtype=torch.float64)
        res += torch.tensor(
            torch.ger(err, err)
...
def _em_transition_matrix(transition_offsets, smoothed_state_means,
    ...
    """
    n_timesteps, n_dim_state, _ = smoothed_state_covariances.shape
    res1 = torch.zeros((n_dim_state, n_dim_state),dtype=torch.float64)
    res2 = torch.zeros((n_dim_state, n_dim_state),dtype=torch.float64)
    for t in range(1, n_timesteps):
        transition_offset = _last_dims(transition_offsets, t - 1, ndims=1)
        res1 += (
            ...
            + torch.ger(smoothed_state_means[t],
                       smoothed_state_means[t - 1])
            - torch.ger(transition_offset, smoothed_state_means[t - 1])
        )
        res2 += (
            smoothed_state_covariances[t - 1]
            + torch.ger(smoothed_state_means[t - 1],
                       smoothed_state_means[t - 1])
        )
    return torch.dot(res1, torch.pinverse(res2))
...
def _em_transition_covariance(transition_matrices, transition_offsets,
    ...
    """
    n_timesteps, n_dim_state, _ = smoothed_state_covariances.shape
    res = torch.zeros((n_dim_state, n_dim_state),dtype=torch.float64)
    for t in range(n_timesteps - 1):
        transition_matrix = _last_dims(transition_matrices, t)
        transition_offset = _last_dims(transition_offsets, t, ndims=1)
        err = torch.tensor(
            torch.tensor(smoothed_state_means[t + 1],dtype=torch.float64)
            - torch.tensor(torch.matmul(transition_matrix, smoothed_state_means[t]),dtype=torch.float64)
            - transition_offset
        )
        Vt1t_A = torch.tensor(
            torch.matmul(pairwise_covariances[t + 1],
                   torch.t(transition_matrix))
        ,dtype=torch.float64)
        res += torch.tensor(
            torch.ger(err, err)
            + torch.tensor(torch.matmul(transition_matrix,
                     torch.matmul(smoothed_state_covariances[t],
                            torch.t(transition_matrix))),dtype=torch.float64)
            + torch.tensor(smoothed_state_covariances[t + 1],dtype=torch.float64)
...
def _em_initial_state_covariance(initial_state_mean, smoothed_state_means,
    ...
    x0_x0 = smoothed_state_covariances[0] + torch.ger(x0, x0)
    return (
        x0_x0
        - torch.ger(initial_state_mean, x0)
        - torch.ger(x0, initial_state_mean)
        + torch.ger(initial_state_mean, initial_state_mean)
    )
...
def _em_transition_offset(transition_matrices, smoothed_state_means):
    ...
    transition_offset = torch.zeros(n_dim_state,dtype=torch.float64)
    for t in range(1, n_timesteps):
        transition_matrix = _last_dims(transition_matrices, t - 1)
        transition_offset += (
            smoothed_state_means[t]
            - torch.dot(transition_matrix, smoothed_state_means[t - 1])
        )
    if n_timesteps > 1:
        return (1.0 / (n_timesteps - 1)) * transition_offset
    else:
        return torch.zeros(n_dim_state,dtype=torch.float64)
...
def _em_observation_offset(observation_matrices, smoothed_state_means,
    ...
    """
    n_timesteps, n_dim_obs = observations.shape
    observation_offset = torch.zeros(n_dim_obs,dtype=torch.float64)
    n_obs = 0
    for t in range(n_timesteps):
        #if not torch.tensor(np.ma.getmask(observations[t]).astype(int)).byte().any():
        #if not observations[t].byte().any():
        ...
        observation_offset += (
            observations[t]
            - torch.dot(observation_matrix, smoothed_state_means[t])
        )
        n_obs += 1
    if n_obs > 0:
...
class KalmanFilter(object):
    ...
    def sample(self, n_timesteps, initial_state=None, random_state=None):
        ...
        states = torch.zeros((n_timesteps, n_dim_state),dtype=torch.float64)
        observations = torch.zeros((n_timesteps, n_dim_obs),dtype=torch.float64)
        ...
        for t in range(n_timesteps):
            if t == 0:
                ...
                states[t] = (
                    torch.dot(transition_matrix, states[t - 1])
                    + transition_offset
                    + rng.multivariate_normal(
                        torch.zeros(n_dim_state,dtype=torch.float64),
            ...
            observations[t] = (
                torch.dot(observation_matrix, states[t])
                + observation_offset
                + rng.multivariate_normal(
                    torch.zeros(n_dim_obs,dtype=torch.float64),
                    observation_covariance.newbyteorder('=')
                )
            )

        return (states, torch.tensor(observations))
    ...
    def filter_update(self, filtered_state_mean, filtered_state_covariance,
        ...
        if observation is None:
            ...
            observation = torch.zeros(n_dim_obs,dtype=torch.float64)
    ...
    def loglikelihood(self, X):
        ...
        return torch.sum(loglikelihoods)
    ...
    def _initialize_parameters(self):
        ...
        defaults = {
            'transition_matrices': torch.eye(n_dim_state,dtype=torch.float64),
            'transition_offsets': torch.zeros(n_dim_state,dtype=torch.float64),
            'transition_covariance': torch.eye(n_dim_state,dtype=torch.float64),
            'observation_matrices': torch.eye(n_dim_obs, n_dim_state,dtype=torch.float64),
            'observation_offsets': torch.zeros(n_dim_obs,dtype=torch.float64),
            'observation_covariance': torch.eye(n_dim_obs,dtype=torch.float64),
            'initial_state_mean': torch.zeros(n_dim_state,dtype=torch.float64),
            'initial_state_covariance': torch.eye(n_dim_state,dtype=torch.float64),
            'random_state': 0,
            'em_vars': [
                'transition_covariance',
                'observation_covariance',
    ...
    def _parse_observations(self, obs):
        ...
        #obs = np.ma.atleast_2d(obs.numpy())
